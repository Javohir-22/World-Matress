...
const questionPopupBackdrop = document.querySelector('.question-popup__backdrop');
const questionNextBtns = document.querySelectorAll('.question-popup__btn--next');
const questionBackBtns = document.querySelectorAll('.question-popup__btn--back');
...
const questionAcceptBtn = document.querySelector('.question-popup__last-btn--accept');
const questionProgress = document.querySelectorAll('.question-popup__progress');

// Response Popup
const responsePopupBackdrop = document.querySelector('.response-popup__backdrop');
...
[numberPopupCardClose, numberPopupBackdrop].forEach(el => el.addEventListener('click', hidePopup.bind(null, numberPopupCard, numberPopupBackdrop)));
...
revealCatalogs(catalogsWrapper, classMoveTop, 0.20);
...
function toggleQuestionPopup() {
  [questionPopupBackdrop, questionPopupCont].forEach(el => el.classList.toggle('hidden'));
}
// Open question popup
headerBtn.firstElementChild.addEventListener('click', toggleQuestionPopup);
// Close question popup
questionPopupCloses.forEach(btn => btn.addEventListener('click', toggleQuestionPopup));
...
document.addEventListener('keydown', function(e) {
  ...
  // Close question popup
  e.key === 'Escape' && !questionPopupCont.classList.contains('hidden') && toggleQuestionPopup();
})
...
function slider() {
  ...
  function goToSlide(slide) {
    questionPopupCards.forEach((s, i) => (s.style.transform = `translateX(${100 * (i - slide)}%)`))
  };
  ...
  function incrementProgress(slide = 0) {
    questionProgress.forEach(s => s.style.width = `${percentPerProgress * (slide + 1)}%`)
  }
  incrementProgress();
}
...
responsePopupBtn.addEventListener('click', hidePopup.bind(null, responsePopupCard, responsePopupBackdrop));